...
#   - multi_school_panel.csv : MMEd + University B 2 校 × 10 年パネル (DID 用)
#
# 想定 RQ:
#   2022 年度の新カリキュラム導入により、MMEd 医学生の
#   CBSE 一発合格率と GPA はどう変化したか?
#
# データ生成方針:
#   - 観察期間: 2017–2026 入学コホート (10 年)
#   - 介入時点: 2022 年 (MMEd のみ)
#   - 介入前 5 年は MMEd と UniB が緩く平行トレンド
#   - 2022 年に MMEd の mean_gpa が +0.5 水準ジャンプ + 緩い post-trend
...
# 年度ごとのコホートサイズ (MMEd は ~150, UniB は ~140)
cohort_size_mmed <- round(rnorm(n_years, mean = 150, sd = 8))
...
mean_gpa_mmed <- round(mean_gpa_mmed, 2)
...
cbse_pass_rate_mmed <- round(pmin(pmax(cbse_pass_rate_mmed, 0), 1), 3)

mmed_panel <- data.frame(
  year                    = years,
  cohort_size             = cohort_size_mmed,
  mean_gpa                = mean_gpa_mmed,
  cbse_pass_rate          = cbse_pass_rate_mmed,
...
# ============================================================
# (2) MMEd + University B パネル (DID 用)
# ============================================================
# 介入校 = MMEd (treated=1)、対照校 = UniB (treated=0)
# UniB は介入を受けないので、MMEd と同じ baseline + 緩いトレンドのみ。
# MMEd 側は (1) と整合させる。
...
# MMEd 側を再生成すると seed 進行で値がズレるので、(1) で作った値を流用する
...
# ============================================================
cat("\n--- ITS sanity check (MMEd, mean_gpa) ---\n")
its_fit <- lm(
...
multi_school_panel$school <- factor(multi_school_panel$school, levels = c("UniB", "MMEd"))
did_fit <- lm(
